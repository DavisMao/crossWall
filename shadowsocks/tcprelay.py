...
#    | 1 | 1 | 1 | 1 | Variable | 2 |
#    +----+-----+-------+------+----------+----------+
#    CMD字段：command的缩写，shadowsocks只用到了：
#    0x01：建立TCP连接
#    0x03：关联UDP请求
#    RSV字段：保留字段，值为0x00；
#    ATYP字段：address type的缩写，取值为：
#    0x01：IPv4
#    0x03：域名
#    0x04：IPv6
#    DST.ADDR字段：destination address
#    的缩写，取值随ATYP变化：
#    ATYP == 0x01：4个字节的IPv4地址
#    ATYP == 0x03：1个字节表示域名长度，紧随其后的是对应的域名
#    ATYP == 0x04：16个字节的IPv6地址
#    DST.PORT字段：目的服务器的端口。

    @shell.exception_handle(self_=True, destroy=True, conn_err=True)
    def _handle_stage_addr(self, data): #建立连接
        if self._is_local:
